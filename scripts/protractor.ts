...
export const isFunction = (node: Node): node is FunctionType =>
...
export function getTransformNode({stepStrategy}: {stepStrategy: 'test' | 'step'}) {
	...
	function transformNode(node: Node, context: ChildrenContext, project: ReturnType<typeof newProject>) {
		...
		if (Node.isCallExpression(node)) {
			...
			const expression = node.getExpression();
			if (protractorPromiseRegExp.test(expression.getType().getText())) {
	...
	function checkDefinitionsAndAddAwaitIfNeeded(node: CallExpression, definitions: Node[], context: ChildrenContext, project: ReturnType<typeof newProject>) {
